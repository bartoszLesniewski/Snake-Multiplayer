...
class Connection:
    def __init__(
        ...
        self._reader = reader
        self._writer = writer
        self._host, self._port = writer.transport.get_extra_info("peername")
        self.session: Session | None = None
        self._log = ConnectionLoggerAdapter(_log, {"connection_key": self.key})
    ...
    async def run(self) -> None:
        while not self._reader.at_eof():
            ...
            try:
                ...
                break
            try:
                handler_func = getattr(self, f"_handle_{msg_type}")
    ...
    async def _on_socket_close(self, exc: OSError | None = None) -> None:
        if exc is None:
            self._log.info("Connection closed cleanly.")
        else:
            self._log.warning("Connection closed abruptly. Reason: %s", str(exc))
    ...
    async def close(self, *, closed_by_client: bool = False) -> None:
        ...
        try:
            ...
            pass
        self._app.connections.pop(self.key, None)
        if not closed_by_client:
            self._log.info("Connection closed by the server.")

    async def _send_message(self, msg_type: MsgType, data: dict[str, Any]) -> None:
        serialized = json.dumps(
            {"type": msg_type.value, "data": data}, separators=(",", ":")
        )
        self._writer.write(f"{serialized}\n".encode())
    ...
    async def _handle_join_session(self, data: dict[str, Any]) -> None:
        ...
        try:
            session = self._app.sessions[code]
        except KeyError:
            await self._send_message(MsgType.INVALID_SESSION, {"exists": False})
            return
        if session.running:
            await self._send_message(MsgType.INVALID_SESSION, {"exists": True})
            return

        previous_session = self.session
        if previous_session is session or self.key in session.players:
            self._log.warning("Client seems to already be in this session.")
    ...
    async def _handle_create_session(self, data: dict[str, Any]) -> None:
        player_name = data["player_name"]
        self.session = await self._app.create_session(self, player_name)
        self._log.info("Created and joined a session with code %r", self.session.code)

    async def _handle_start_session(self, data: dict[str, Any]) -> None:
        if self.session is None:
            await self._send_message(MsgType.NOT_IN_SESSION, {})
            return
        ...
        if self.session.owner.conn is not self:
            await self._send_message(MsgType.NOT_SESSION_OWNER, {})
            return

        await self.session.start()
        self._log.info("Started a session with code %r", self.session.code)
    ...
    async def _handle_input(self, data: dict[str, Any]) -> None:
        ...
        self._log.info("Updated player's direction to %s.", new_direction.name)
    ...
    async def send_session_join(self, session: Session, player: SessionPlayer) -> None:
        ...
        if player.key == self.key:
            self._log.info("Joined a session with code %r", session.code)
            payload["players"] = [
    ...
    async def send_session_leave(self, session: Session, key: str) -> None:
        if key == self.key:
            self._log.info("Left a session with code %r", session.code)
            self.session = None
    ...
    async def send_session_state_update(
        ...
        if not self._writer.is_closing():
            await self._send_message(MsgType.SESSION_STATE_UPDATE, state)
